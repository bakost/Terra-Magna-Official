...
class protect(commands.Cog):
    ...
    @commands.command(aliases=['protect'], pass_context=True)
    async def __protect(self, ctx, *,  args=None):
        member = ctx.message.author

        if args is None:
            await ctx.send(f"**{member.mention}**, отсутствуют аргументы! Пример команды: !protect [Название морской провинции] [Количество галеонов] [Количество каравелл]")
            return
        ...
        for i in range(0, len(reco1)):
            ...
            if (temp == maritime) and (temp2 == ctx.author.id) and (temp3 == 2):
                await ctx.send(f"**{member.mention}**, Вы уже охотитесь на пиратов в этой морской провинции!")
                return
        ...
        cursor.execute(f"INSERT INTO {ctx.author.guild.id}_grab(name, iduser, maritime, galeons, karavells, type) VALUES ('{member}',{member.id},'{maritime}',{galeons},{karavells},2)")

        connection.commit()
        await ctx.send(f"**{member.mention}**, Вы начали охоту на пиратов в \"{maritime}\"!")


    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.command(aliases=['unprotect'], pass_context=True)
    async def __unprotect(self, ctx, *,  args=None):
        member = ctx.message.author

        if args is None:
            await ctx.send(f"**{member.mention}**, отсутствуют аргументы! Пример команды: !unprotect [Название морской провинции] [Количество галеонов] [Количество каравелл]")
            return
        ...
        for i in range(0, len(reco1)):
            ...
            if (temps == maritime) and (temps3 == ctx.author.id) and (temps4 == 2):
                cursor.execute(f"SELECT fleet FROM `{ctx.author.guild.id}` WHERE iduser = {ctx.author.id}")
                fleet = cursor.fetchall()
                ...
                cursor.execute(f"SELECT galeons FROM {ctx.author.guild.id}_grab WHERE maritime = '{maritime}' and type = 2")
                galeons = cursor.fetchone()[0]

                cursor.execute(f"SELECT karavells FROM {ctx.author.guild.id}_grab WHERE maritime = '{maritime}' and type = 2")
                ...
                cursor.execute(f"DELETE FROM {ctx.author.guild.id}_grab WHERE maritime = '{maritime}' and type = 2")

                connection.commit()
                await ctx.send(f"**{member.mention}**, Вы закончили охоту на пиратов в \"{maritime}\"!")
                return

        await ctx.send(f"**{member.mention}**, Вы не проводили охоту на пиратов в данной морской провинции!")
